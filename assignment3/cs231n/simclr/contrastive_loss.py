...
def l_naive(z_i,z_j,i,out,tau,N):
    l_1=-sim(z_i,z_j)/tau
    fm=0
    for k in range(2*N):
        if k!=i:
            fm+=torch.exp(sim(z_i,out[k])/tau)
    fm=torch.log(fm)
    l_1+=fm
    return l_1
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss+=l_naive(z_k,z_k_N,k,out,tau,N)+l_naive(z_k_N,z_k,k+N,out,tau,N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs=(out_left*out_right).sum(axis=1,keepdim=True)/(torch.linalg.norm(out_left,dim=1,keepdim=True)*torch.linalg.norm(out_right,dim=1,keepdim=True))
...
def compute_sim_matrix(out):
    ...
    out_norm=1/torch.linalg.norm(out,dim=1,keepdim=True)
    sim_matrix=(out@out.T)*out_norm*(out_norm.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.log(exponential.sum(axis=1,keepdim=True))
    ...
    sum1=-sim_matrix[torch.arange(N),torch.arange(start=N,end=2*N)].sum()/tau
    sum2=-sim_matrix[torch.arange(start=N,end=2*N),torch.arange(N)].sum()/tau
