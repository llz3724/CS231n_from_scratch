...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
        ...
        dx[:,i,:],dprev_h,tmp_dWx,tmp_dWh,tmp_db=rnn_step_backward(dnext_h,cache[i])
        dWh+=tmp_dWh
        dWx+=tmp_dWx
        db+=tmp_db
    dh0=dprev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    H=prev_h.shape[1]
    a=x@Wx+prev_h@Wh+b
    a_i,a_f,a_o,a_g=np.split(a,indices_or_sections=4,axis=1)
    i,f,o,g=sigmoid(a_i),sigmoid(a_f),sigmoid(a_o),np.tanh(a_g)
    next_c=f*prev_c+i*g
    next_h=o*np.tanh(next_c)
    cache=[x,prev_h,prev_c,next_c,Wx,Wh,i,f,o,g,a_i,a_f,a_o,a_g]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x,prev_h,prev_c,next_c,Wx,Wh,i,f,o,g,a_i,a_f,a_o,a_g=cache
    dnext_c_tot=dnext_c+dnext_h*o*(1-np.tanh(next_c)**2)
    dprev_c=dnext_c_tot*f
    di=g*dnext_c_tot
    df=prev_c*dnext_c_tot
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        x_t=x[:,t,:]
        prev_h,prev_c,tmp_cache=lstm_step_forward(x_t, prev_h, prev_c, Wx, Wh, b)
        cache.append(tmp_cache)
        h[:,t,:]=prev_h
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        ...
        dx[:,t,:], dprev_h, dprev_c, tmp_dWx, tmp_dWh, tmp_db=lstm_step_backward(dnext_h, dnext_c, tmp_cache)
        dWx+=tmp_dWx
        dWh+=tmp_dWh
        db+=tmp_db
    dh0=dprev_h
