...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        C,H,W=input_dim
        F=num_filters
        self.params['W1']=np.random.normal(loc=0,scale=weight_scale,size=(F,C,filter_size,filter_size))
        self.params['b1']=np.zeros(F)

        pool_height=2
        pool_width=2
        pool_stride=2
        H_pool=1+(H-pool_height)//pool_stride
        W_pool=1+(W- pool_width)//pool_stride
        self.params['W2']=np.random.normal(loc=0,scale=weight_scale,size=(F*H_pool*W_pool,hidden_dim))
